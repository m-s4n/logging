...
public class RequestSpoolWorker : BackgroundService
{
    private readonly IServiceScopeFactory _scopeFactory;
    private readonly LogSpoolOptions _opt;
    private readonly JsonSerializerOptions _json = new(JsonSerializerDefaults.Web);

    public RequestSpoolWorker(IServiceScopeFactory scopeFactory, IOptions<LogSpoolOptions> opt)
    {
    ...
    {
        var dir = Path.Combine(_opt.RootPath, "request");
    ...
    private async Task ProcessFile(string file, CancellationToken ct)
    {
        ...
        var batch = new List<RequestLog>(_opt.BatchSize);
        ...
        while (!sr.EndOfStream && !ct.IsCancellationRequested)
        {
            ...
            RequestLog? item;
            try { item = JsonSerializer.Deserialize<RequestLog>(line, _json); }
            catch { continue; }
